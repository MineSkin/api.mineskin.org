...
module.exports.getAvailableAccount = function (req, res, cb) {
    ...
    Account.findOne({enabled: true, requestServer: {$in: [null, config.server]}, lastUsed: {'$lt': (time - 30)}, errorCounter: {'$lt': 10}}).sort({lastUsed: 1, lastSelected: 1, errorCounter: 1}).exec(function (err, account) {
        if (err) return console.log(err);
        if (!account) {
